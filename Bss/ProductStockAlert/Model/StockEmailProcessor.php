...
class StockEmailProcessor
{
            ...
            foreach ($affectedSkuWebsiteCode as $sku => $pidAndWebsite) {
                try {
                    ...
                    if (in_array($product->getTypeId(), ['bundle', 'configurable', 'grouped'])) {
                        ...
                        $qty = 0;
                        $parentStock = $this->getStockStatus($pid, $stockId);
                        $childStock = false;
                        foreach ($childs as $child) {
                            if ($this->getStockStatus($child->getId(), $stockId)) {
                        ...
                        $status = $this->getStockStatus($pid, $stockId);
    ...
    public function getStockStatus($productId, $stockId)
    {
        ...
        $select = $connection->select()
            ...
            ->where('css.product_id=?', $productId)
            ->where('css.stock_id=?', $stockId);
