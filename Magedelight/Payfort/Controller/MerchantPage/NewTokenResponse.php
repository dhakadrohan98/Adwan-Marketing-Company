    ...
    public function execute()
    {
        ...
        if ($response['response_code']==self::TOKENSUCCESS) {
            try {
                ...
                $CardExits =  $this->paymentTokenFactory->create()->getCollection()->addFieldToFilter('public_hash', ["eq" => $vaultCard['public_hash']]);
                if (count($CardExits->getData()) == 0) {
                    ...
                    $entityId = $CardExits->getData()[0]['entity_id'];
                    $modelUpdate = $model->load($entityId);
                    $modelUpdate->setIsActive(1);
                    $modelUpdate->setIsVisible(1);
                    $modelUpdate->save();
